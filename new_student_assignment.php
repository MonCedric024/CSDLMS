...
if (isset($subject)) {
    $stmt = $conn->prepare("SELECT * FROM assignment WHERE subject = ?");
    ...
    <div class="accordion_box" style="margin-left: 550px;">
        ...
        if (isset($result) && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<p><strong>Title: </strong>" . htmlspecialchars($row['description']) . "</p>";
                echo "<p><strong>Due Date: </strong>" . htmlspecialchars($row['due_time']) . "</p>";
                echo "<a href='new_view_assignment.php?assignment_id=" . htmlspecialchars($row['assignment_id']) . "&subject=" . htmlspecialchars($subject) . "'><button class='view_btn'>View Assignment</button></a>";
                echo "<hr>";
            }
        } else {
